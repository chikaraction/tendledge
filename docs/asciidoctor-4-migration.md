...
- ~~表の `<col width="50%">` が DOMPurify のサニタイズを通過するか~~ →
  **通過する**。`sanitize.test.ts` に特性化テストを追加して固定済み。
- ~~v4 の変換速度~~ → **デバウンス既定値(300ms)の見直しは不要**。
  `load()` + `doc.convert()` の実測(warm、中央値)は
  1.2KB で 3ms / 6.2KB で 8ms / 25KB で 26ms(最大 59ms)。
  想定される文書サイズでは 300ms のデバウンス内に十分収まる。
- `stylesheet: false` を外すと v4 は既定 CSS(約 29KB)を `<style>` に埋め込むうえ、
  Google Fonts への外部 `<link>` も出力する。この指定はエクスポートした HTML から
  外部リクエストを発生させないためにも効いている(実測で確認)。
