...
## 実装前の擦り合わせ結果(2026-07-07 ユーザー承認済み)

1. **Draw.io はベストエフォートで含める** — kroki.io 停止中はエラー表示になる旨を
   サンプルに明記し、動作保証は self-host / 復旧後とする
2. **図の挿入方式は `<img>` data URI で確定** — 図内テキストの選択・コピー不可は許容
3. オプトイン OFF 時の設定導線ヒントは実装時の見た目で実装者判断とする
